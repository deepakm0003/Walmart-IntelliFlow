...
export const getAllInventoryItems = (): Array<{ name: string; category: string; baseStock: number; basePrice: number }> => {
  // Base items from this file
  const baseItems = [
    { name: 'Cold Beverages - 500ml', category: 'Beverages', baseStock: 5000, basePrice: 25 },
    { name: 'Cooking Oil - 1L', category: 'Groceries', baseStock: 3000, basePrice: 120 },
    { name: 'Rice - 25kg', category: 'Staples', baseStock: 2000, basePrice: 1200 },
  ...
  // Items from weatherService
  const weatherItems = (weatherService as any).getAllPossibleItems?.() || [];
  // Merge and deduplicate by name
  const all = [...baseItems];
  for (const wi of weatherItems) {
    if (!all.some(bi => bi.name === wi.name)) {
      all.push({ name: wi.name, category: wi.category, baseStock: 1000, basePrice: 100 });
    }
  }
  return all;
};